...
void lcdLayout(bool concise){
  ...
  if (!concise){
      ...
      lcd.setCursor(8, 0);
      lcd.print("uSv ");
      lcd.print(uSvConvert(countpermin));
      lcd.setCursor(8, 1);
      lcd.print("ELP ");
      if(trigger_elapsed<999)
      lcd.print(trigger_elapsed);
      else lcd.print("1k+");
      lcd.print("s");
  }else{
    lcd.setCursor(0, 0);
...
void loop()
{
  ...
  if (millis() - refreshTimer > 1000)
  {
    ...
    {
      lcdLayout(1); // true = concise reading without descriptor to prevent display out of bounds, intended for long duration measurement or 0802 screen.
      Serial.print("TOTAL ");
      Serial.print(count);
